...
# 高级索引：当选择对象obj是一个非元组序列对象、是一个ndarray(数据类型为整数或布尔)、或者是一个至少包含一个序列对象或ndarray(数据类型为整数或布尔)的元组时，会触发高级索引。有两种类型的高级索引：整数数组索引和布尔数组索引。

# 在程序中处理可变数量的索引：索引语法功能非常强大，但在处理可变数量的索引时却有限制。例如，如果您想编写一个函数，该函数可以处理具有不同维数的参数，而又不想为每个可能维度编写特殊情况代码，如何做到这一点？如果向索引提供了一个元组，该元组将被解释为索引列表。因此，可以使用代码来构建包含任意数量的索引的元组，然后在索引中使用这些元组。
