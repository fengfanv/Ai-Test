...
#-----------------
train_acc_list = []#每次epoch时，记录，当前这个神经网络，对于训练数据的识别精度（不知道epoch是啥，看书，看4.5.3基于测试数据的评价）
test_acc_list = []#每次epoch时，记录，当前这个神经网络，对于测试数据的识别精度
iter_per_epoch = max(train_size / batch_size, 1)# 平均每个epoch的重复次数（可以理解为，记录识别精度的重复次数）（总数据60000个，每次mini-batch随机抽取100个，60000/100=600，每当iters_num循环，循环到n倍的600次时，就记录一下识别精度。如果仅循环10次，100次，根本就不会去统计，识别精度）
